# 📝 프로젝트 소개
>3개월간의 'K-Shield 주니어 6기' 교육과정에서 진행한 중국 모바일 어플리케이션의 개인정보 수집과 활용에 관한 분석 및 자동화 방안 연구 프로젝트입니다.         
...
이 상황에서 저는 PL과 얘기를 주고받으며 Javascript 코드를 공부하고 구글링을 통해 얻은 코드들을 한줄 한줄 분석한 결과를 문서로 만들어 팀원들에게 공유해 주었습니다.
 ...
 - 시간이 촉박하여 이떄 저는 PM으로서 참여 의지가 부족한 몇몇 인원들의 동기 부여를 이끌어 내는것이 멘탈적으로 조금 힘들었지만, 돌아보면 이 경험을 통해 제 스스로가 한단계 더 성장한것 같습니다.

 - 마지막으로 프로젝트 초기에 간과했던 것이 있다면 프로젝트 주제 선정시 팀원이 10명이라고 해서 반드시 10명이서 해야할법한 프로젝트로 방향을 잡고 조사하면 안된다는 깨달음을 었었습니다. 10명의 팀원의 개인 역량과 프로젝트 경험도 서로 다르기 때문에 이러한 상황에 대한 파악을 제대로 하여 팀원들 모두가 흥미를 가지고 열정적으로 참여할 수 있는 수준의 프로젝트를 선정했으면 좋았을 것 같다는 생각을 하였습니다.
